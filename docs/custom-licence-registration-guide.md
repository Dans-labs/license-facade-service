...
This guide shows how to register a new custom licence in the License Facade Service (LFS) using `curl`, Swagger UI, or Postman. It includes complete examples for all three registration scopes:
...
- the latest PostgreSQL schema installed from `docker/postgres-init/001-lfs-schema.sql`;
...
```bash
psql "$CUSTOM_LICENCE_REGISTRATION_DATABASE_URL" \
  -v ON_ERROR_STOP=1 \
  -f docker/postgres-init/001-lfs-schema.sql
```

Docker Compose runs this automatically on the first startup of an empty PostgreSQL data directory. For an external PostgreSQL service, run the command above once yourself.
